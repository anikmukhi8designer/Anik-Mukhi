...
const Footer: React.FC<FooterProps> = ({ footer, siteInfo, navigation, onViewChange }) => {
            ...
            <div className="flex flex-col gap-2">
              ...
              {navigation.map(nav => (
                <a
                  ...
                  href={nav.href}
                  onClick={(e) => {
                    if (['WORK', 'ABOUT'].includes(nav.label)) {
                      e.preventDefault();
                      onViewChange(nav.href as 'work' | 'about');
                    }
                  }}
                  className="group relative overflow-hidden h-5 uppercase text-[10px] tracking-widest text-neutral-500 cursor-pointer"
                >
                  <div className="relative flex flex-col transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:-translate-y-full">
